...
- Rejections page (`/rejections`): the Journal's counterpart — the mail that never got in. A relay's most dangerous state is silently not relaying, so every refused submission is recorded (aggregated per client, reason and sender domain, with a count and how long it has been failing) instead of being only a log line nobody reads. Clients inside your own configured networks are listed individually with the exact gate that refused them and **one-click fixes** (accept the sender domain, allow the IP, or ignore); everything else — the port-25 background noise — is only counted, never reported. A device from a configured network that has been failing for over a day becomes a warning in the daily self-check and the daily report
...
- Fully qualified EHLO/HELO on outbound deliveries — announces the public hostname (Settings → Sending identity), overridable per send connector for multi-egress setups; delivery pauses loudly (queue + self-check) rather than announce an unqualified name that strict receivers reject
...
- MCP server (`/mcp`) so an AI assistant (e.g. Claude Code) can troubleshoot and fix the relay — API-key
  authenticated, capability-scoped, fully audited; setup guide on the admin UI's **MCP** page
...
For AI assistants, the same keys authenticate the built-in **MCP server** at `/mcp` (streamable HTTP): diagnostic tools (queue, delivery attempts, rejections, self-check, audit search) and routine fixes (accept a domain, allow an IP, manage suppressions, re-queue), with message bodies and all account/tenant/key administration deliberately excluded. The admin UI's **MCP** page has the client setup, including a ready `claude mcp add` command.
