...
public static class SystemEmailHtml
{
    ...
    /// <summary>Card width for transactional mail — a comfortable measure for a few lines of prose.</summary>
    private const int DefaultCardWidth = 640;

    /// <summary>
    /// Card width for <see cref="SystemEmailContent.WideLayout"/>. Together with the reduced side padding
    /// it gives the monospace block ~110 characters at 13px, enough for the daily report's longest lines
    /// (health-check findings that repeat a fully-qualified domain), and still fits an Outlook reading
    /// pane without horizontal scrolling.
    /// </summary>
    private const int WideCardWidth = 860;

    public static string Render(SystemEmailContent content)
    {
        var html = new StringBuilder();
        var cardWidth = content.WideLayout ? WideCardWidth : DefaultCardWidth;
        var sidePadding = content.WideLayout ? 24 : 32;
        ...
        html.Append($"<table role=\"presentation\" width=\"{cardWidth}\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:{cardWidth}px;width:100%;background-color:#ffffff;border:1px solid #e4e4e7;border-radius:8px;font-family:Aptos,'Segoe UI',Arial,sans-serif;\">");

        // Branding header
        html.Append($"<tr><td style=\"padding:20px {sidePadding}px;border-bottom:1px solid #e4e4e7;\">");
        html.Append($"<span style=\"font-size:16px;font-weight:600;color:#18181b;letter-spacing:0.02em;\">{Encode(AppName)}</span>");
        html.Append("</td></tr>");

        // Body
        html.Append($"<tr><td style=\"padding:32px {sidePadding}px;\">");
        ...
        {
            html.Append($"<tr><td style=\"padding:16px {sidePadding}px;border-top:1px solid #e4e4e7;\">");
